...
async function query(db: Database, name: string, data: string, env: Env): Promise<{ result: BytesLike; validUntil: number }> {
...
export function makeServer(signer: ethers.utils.SigningKey, db: Database | Promise<Database>, env: Env) {
			...
			func: async ([encodedName, data]: Result, request) => {
				...
				try {
					...
					let messageHash = ethers.utils.solidityKeccak256(
						...
						['0x1900', request?.to, validUntil, ethers.utils.keccak256(request?.data || '0x'), ethers.utils.keccak256(result)]
					);
...
export function makeApp(signer: ethers.utils.SigningKey, path: string, db: Database | Promise<Database>, env: Env) {
